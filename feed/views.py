...
class Feed(APIView):
    ...
    def get(self, request):
        ...
        queryset = Media.objects.filter(
            media_type=MEDIA_TYPES.VIDEO, is_public=True, is_deleted=False
        )

        queryset = queryset.select_related(
            "uploaded_by", "place", "place__city"
        ).order_by("-created_at")
        serializer = MediaFeedSerializer(
        ...
        if serializer.is_valid():
            ...
            Activity.create_activity(
                ...
                activity_type=(
                    ...
                    else ActivityType.VIDEO_UPLOAD
                ),  # Using same type for now
                content_object=media,
            )

            response_serializer = MediaFeedSerializer(
...
class MediaFeedView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Media.objects.filter(is_public=True, is_deleted=False)
...
class UserMediaView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Media.objects.filter(uploaded_by=self.request.user, is_deleted=False)
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            return Media.objects.filter(
                Q(is_public=True) | Q(uploaded_by=self.request.user), is_deleted=False
            ).select_related("uploaded_by", "place", "place__city")
        else:
            return Media.objects.filter(
...
class MediaUpdateView(generics.UpdateAPIView):
    ...
    def get_queryset(self):
        ...
        return Media.objects.filter(uploaded_by=self.request.user, is_deleted=False)
...
class MediaDeleteView(generics.DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Media deleted successfully"}, status=status.HTTP_200_OK
        )
...
class PlacesListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Place.objects.filter(is_deleted=False)
            .select_related("city")
            .order_by("name")
        )
