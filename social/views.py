...
from .serializers import ActivitySerializer, ActivityStatsSerializer, FollowSerializer
...
class ActivityFeedView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Activity.objects.filter(target_user=user)
...
class MarkActivitiesAsReadView(APIView):
    ...
    def post(self, request):
        ...
        if activity_ids:
            ...
            Activity.objects.filter(id__in=activity_ids, target_user=user).update(
                is_read=True
            )
        else:
        ...
        return Response(
            {"message": "Activities marked as read"}, status=status.HTTP_200_OK
        )
...
class FollowUserView(APIView):
    ...
    def post(self, request, user_id):
        ...
        if created:
            ...
            return Response(
                {"message": "Already following this user"}, status=status.HTTP_200_OK
            )
...
class UnfollowUserView(APIView):
    ...
    def post(self, request, user_id):
        ...
        try:
            ...
            return Response(
                {"error": "Not following this user"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ToggleLikeView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Content object not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if created:
            ...
            return Response(
                {"message": "Liked successfully", "liked": True},
                status=status.HTTP_201_CREATED,
            )
        else:
            like.delete()
            return Response(
                {"message": "Unliked successfully", "liked": False},
...
class AddCommentView(APIView):
    ...
    def post(self, request):
        ...
        if not content_type_id or not object_id or not text:
            return Response(
                {"error": "content_type_id, object_id, and text are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        comment = Comment.objects.create(
            user=request.user, content_type=content_type, object_id=object_id, text=text
        )
